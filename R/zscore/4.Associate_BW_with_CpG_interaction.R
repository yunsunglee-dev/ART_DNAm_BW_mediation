...
#role="child";exposure="non_fresh";method="bon";alpha=0.05
...
### Define Standardized BW.
#hist(info$VEKT)
#cat("Print the dist of standardized BW\n")
#print(summary(info$VEKT))
#info$VEKT_s=scale(info$VEKT,center = T,scale = T)
#print(summary(info$VEKT_s))
...
work_horse=function(i){
    model3=nlme::lme(as.formula(paste(outcome,"~",target_cpgs[i],"+",exposure,"+",target_cpgs[i],"*",exposure,"+",
...
work_horse=function(i){
    model3=nlme::lme(as.formula(paste(outcome,"~",target_cpgs[i],"+",
                                      paste(adj_vars,collapse = "+"),
                         sep="")),random=~1|c_AMP_Plate,data=info,na.action="na.exclude")
    tab=summary(model3)$tTable
...
feather::write_feather(info,paste("data/info_",role,"_",exposure,".feather",sep=""))
