...
boot=function(b){
    ...
    set.seed(b)
    info_boot=na.omit(info[sample(1:nrow(info),nrow(info),replace = T),c("VEKT",exposure,adj_vars,cpgs_interest,"c_AMP_Plate")])
    ...
    myfor1=as.formula(paste("VEKT~",exposure,"+",paste(cpgs_interest,collapse="+"),"+",paste(adj_vars,collapse = "+"),sep=""))
    model1=nlme::lme(myfor1, random=~1|c_AMP_Plate, data=info_boot)
    tab=summary(model1)$tTable
